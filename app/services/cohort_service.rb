class CohortService
  ...
  def self.avg_by_age_all_cohorts(measurement:, max_age: 30)
    avgs_by_cohort = {}
    Cohorts.each do |cohort|
      avgs_by_cohort[cohort.id] = CohortService.avg_by_age_for(cohort_id: cohort.id,
  ...
  def self.avg_by_age_for(cohort: , measurement: , max_age: 30)

    query = "
    SELECT
      avg(#{sanitize_measurement(measurement)}_count),
      age
