class MeasurementService
  ...
  def self.subqueries
    [
      ...
      {name: :member_group_visits, table: :group_visits, relation: GroupVisit.where("member = true and group_id = outer_groups.id")},
      {name: :organisation_visits, table: :organisation_visits, relation: OrganisationVisit.where('organisation_id = outer_groups.id')},
      {name: :member_organisation_visits, table: :organisation_visits, relation: OrganisationVisit.where("member = true AND organisation_id = outer_groups.id")}
    ]
  end
end
